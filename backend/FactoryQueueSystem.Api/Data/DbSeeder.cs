...
public static class DbSeeder
{
    public static async Task SeedAsync(IServiceProvider services)
    {
        ...
        var driver = await db.Users.FirstOrDefaultAsync(x => x.Email == driverEmail);
        if (driver == null)
        {
            driver = new User
            {
        ...
        var vehicle = await db.Vehicles.FirstOrDefaultAsync(x => x.PlateNumber == "34 FQS 001");
        if (vehicle == null)
        {
            vehicle = new Vehicle
            {
        ...
        var hasActiveShipment = await db.Shipments
            .AnyAsync(x => x.VehicleId == vehicle.Id && x.Status != ShipmentStatus.Completed);
